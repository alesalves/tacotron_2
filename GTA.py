...
def GTA_Synthesis(output_directory, checkpoint_path, n_gpus,
    ...
    for i, batch in enumerate(train_loader):
        ...
        _, input_lengths, _, _, output_lengths, _, _  = batch # output_lengths: orgnal mel length
        ...
        _, _, mel_outputs_postnet, _, _  = model(x)
        mel_outputs_postnet = mel_outputs_postnet.data.cpu().numpy()

        for k in range(batch_size):
            wav_path = org_audiopaths[k]
            mel_path = output_directory + 'mels/' + os.path.basename(mel_paths[k])+'.npy'
            map = "{}|{}\n".format(wav_path,mel_path)
            f.write(map)
            # To do: size mismatch
            diff = output_lengths[k] - (input_lengths_[k] / hparams.hop_length)
            mel = mel_outputs_postnet[k,:,:output_lengths[k]]#mel_outputs_postnet[k,:,:output_lengths[k]-diff]
            if diff != 0: print(wav_path, input_lengths_[k],input_lengths[k], output_lengths[k], mel.shape, diff)
